...
def test_example_8_empty_input(assert_parses):
    result = translate_records([], VMAP, doc_id="empty-doc")
...
def test_thousands_commas():
    assert normalize_threshold("1,941") == "1941."


def test_precision_preserved():
    assert normalize_threshold("38.0") == "38.0"
    assert normalize_threshold("619.08") == "619.08"


def test_integer_trailing_dot():
    assert normalize_threshold("1420") == "1420."


def test_leading_dot_gets_zero():
    assert normalize_threshold(".5") == "0.5"


def test_ascii_scientific():
    assert normalize_threshold("4.7e-3") == "0.0047"
    assert normalize_threshold("1.0×10⁻²") == "0.010"
...
def test_suggest_pv_name():
    assert suggest_pv_name("Feed/Steam Flow Mismatch") == "feedsteamflowmismatch"
    assert suggest_pv_name("Vessel Coolant Level - Low-Low") == "vesselcoolantlevellowlow"
...
def test_header_contains_version_doc_and_counts():
    records = [rec("Steam Flow", "f", [cond("BELOW_MIN", "10")])]
    result = translate_records(records, VMAP, doc_id="ML17056A228_p3")
...
def test_determinism_byte_identical():
    ...
    a = translate_records(records, VMAP, doc_id="d")
    b = translate_records(records, VMAP, doc_id="d")
...
def test_accounting_error_on_lost_record(monkeypatch):
    # simulate the "forgot a code path" bug class: a result translate_records() does
    ...
    with pytest.raises(AccountingError):
        translate_records([rec("Steam Flow", "f", [cond("BELOW_MIN", "10")])], VMAP)
